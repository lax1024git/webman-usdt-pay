...
import { ElMessage, ElTable, ElTableColumn, ElButton, ElPagination, ElTag } from 'element-plus'
import { ContentWrap } from '@/components/ContentWrap'
import { getPayWebhookLogListApi, retryPayWebhookApi } from '@/api/pay'

const loading = ref(false)
const list = ref<any[]>([])
const total = ref(0)
const queryParams = ref({ page: 1, limit: 20, order_no: '' })

const loadData = async () => {
  loading.value = true
  try {
    const res = await getPayWebhookLogListApi(queryParams.value)
...
const handleRetry = async (row: any) => {
  await retryPayWebhookApi(row.id)
  ...
  <ContentWrap title="回调日志">
    <ElTable v-loading="loading" :data="list" border stripe>
      ...
      <ElTableColumn label="操作" width="100" fixed="right">
        <template #default="{ row }">
          <ElButton v-if="row.status !== 'success'" link type="primary" @click="handleRetry(row)">重试</ElButton>
        </template>
      </ElTableColumn>
    </ElTable>
    <ElPagination
