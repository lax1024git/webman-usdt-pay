...
const fetchDetailUserApi = async () => {
  ...
  userInfo.value = {
    ...
    roleList: (data.roles ?? []).map((r: any) => r.name || r.slug || r)
  }
}
fetchDetailUserApi()
...
<template>
  <div class="flex w-100% h-100%">
    <ContentWrap :title="t('个人信息')" class="w-400px">
    ...
    <ContentWrap :title="t('基本资料')" class="flex-[3] ml-20px">
      <ElTabs v-model="activeName">
        <ElTabPane :label="t('基本信息')" name="first">
          <EditInfo :user-info="userInfo" />
        </ElTabPane>
        <ElTabPane :label="t('修改密码')" name="second">
          <EditPassword :user-id="userInfo?.id" />
        </ElTabPane>
        <ElTabPane :label="t('谷歌验证')" name="third">
          <BindGoogleAuth />
        </ElTabPane>
      </ElTabs>
    </ContentWrap>
  ...
  <Dialog v-model="dialogVisible" :title="t('修改头像')" width="800px">
    <UploadAvatar ref="uploadAvatarRef" :url="userInfo?.avatarUrl || defaultAvatar" />

    <template #footer>
      <ElButton type="primary" :loading="avatarLoading" @click="saveAvatar"> {{ t('保存') }} </ElButton>
      <ElButton @click="dialogVisible = false">{{ t('关闭') }}</ElButton>
    </template>
  </Dialog>
</template>
