...
import { merchantDepositListApi, merchantDepositShowApi } from '@/api/merchantPortal'
...
const loadData = async () => {
  loading.value = true
  try {
    const res = await merchantDepositListApi(queryParams.value)
...
const openDetail = async (row: any) => {
  const res = await merchantDepositShowApi(row.id)
...
<template>
  <ContentWrap title="入金订单">
    ...
    <ElTable v-loading="loading" :data="list" border stripe>
      ...
      <ElTableColumn prop="paid_amount" label="实付" width="110" />
    ...
    <ElDialog v-model="detailVisible" title="入金详情" width="640px">
      <ElDescriptions v-if="detail" :column="2" border>
        ...
        <ElDescriptionsItem label="实付">{{ detail.paid_amount }}</ElDescriptionsItem>
        <ElDescriptionsItem label="状态">{{ statusMap[detail.status] || detail.status }}</ElDescriptionsItem>
        <ElDescriptionsItem label="确认数">{{ detail.confirmations }}</ElDescriptionsItem>
        <ElDescriptionsItem label="收款地址" :span="2">{{ detail.deposit_address }}</ElDescriptionsItem>
